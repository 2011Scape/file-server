...
internal class FileServerTest {
    ...
    @Test
    fun `Encode header for prefetch request`() =
        runTest {
    ...
    @Test
    fun `Encode header for priority request`() =
        runTest {
            val server = spyk(FileServer(mockk(), byteArrayOf()))
            val write: ByteWriteChannel = mockk()
    ...
    @Test
    fun `Encode small single sector`() =
        runTest {
    ...
    @Test
    fun `Encode fixed size single sector`() =
        runTest {
            val data = byteArrayOf(1, 2, 3, 4, 5)
            val server = FileServer(mockk(), byteArrayOf())
            val write: ByteWriteChannel = mockk()
